...
class InputForm extends Component {
  ...
  submit(e) {
    ...
    return axios({method: 'get', url: `http://localhost:5000/api/recipes/?ingredients=${this.state.ingredients}&food=${this.state.food}&p=${this.state.page}`}).then((results) => {
      ...
      this.setState({oldingredients: this.state.ingredients, oldfood: this.state.food, oldpage: this.state.page,})
  ...
  PrevPage(e) {
    ...
    return axios({method: 'get', url: `http://localhost:5000/api/recipes/?ingredients=${this.state.oldingredients}&food=${this.state.oldfood}&p=${this.state.oldpage}`}).then((results) => {
      return results.data.results;
    }).then((data) => {
      let recipes = data.map((recipe, index) => {
        return (<div className='col-md-4' key={index}>
